...
final class EngineVector3Tests {
    ...
    public func testConstructorMethods () {
        ...
        assertEqual (vectorEmpty, vectorZero, "Vector3 Constructor with no inputs should return a zero Vector3.")
    }
    ...
    public func testAxisMethods () {
        ...
        assertEqual (vector.maxAxisIndex (), Vector3.Axis.z.rawValue, "Vector3 maxAxisIndex should work as expected.")
        assertEqual (vector.minAxisIndex (), Vector3.Axis.x.rawValue, "Vector3 minAxisIndex should work as expected.")
    ...
    public func testInterpolationMethods () {
        ...
        assertEqual (vector1.lerp (to: vector2, weight: 0.5), Vector3 (x: 2.5, y: 3.5, z: 4.5), "Vector3 lerp should work as expected.")
        ...
        assertEqual (Vector3 ().slerp (to: Vector3 (), weight: 0.5), Vector3 (), "Vector3 slerp with both inputs as zero vectors should return a zero vector.")
        assertEqual (Vector3 ().slerp (to: Vector3 (x: 1, y: 1, z: 1), weight: 0.5), Vector3 (x: 0.5, y: 0.5, z: 0.5), "Vector3 slerp with one input as zero should behave like a regular lerp.")
        assertEqual (Vector3 (x: 1, y: 1, z: 1).slerp (to: Vector3 (), weight: 0.5), Vector3 (x: 0.5, y: 0.5, z: 0.5), "Vector3 slerp with one input as zero should behave like a regular lerp.")
        ...
        assertEqual (vector1.cubicInterpolate (b: vector2, preA: Vector3 (), postB: Vector3 (x: 7, y: 7, z: 7), weight: 0.5), Vector3 (x: 2.375, y: 3.5, z: 4.625), "Vector3 cubicInterpolate should work as expected.")
        ...
        assertEqual (Vector3 (x: 1, y: 0, z: 0).moveToward (to: Vector3 (x: 10, y: 0, z: 0), delta: 3), Vector3 (x: 4, y: 0, z: 0), "Vector3 moveToward should work as expected.")
    }

    public func testLengthMethods () {
        let vector1: Vector3 = Vector3 (x: 10, y: 10, z: 10)
        let vector2: Vector3 = Vector3 (x: 20, y: 30, z: 40)
        assertEqual (vector1.lengthSquared (), 300, "Vector3 lengthSquared should work as expected and return exact result.")
        assertApproxEqual (vector1.length (), 10 * Double.sqrt3, "Vector3 length should work as expected.")
        assertEqual (vector2.lengthSquared (), 2900, "Vector3 lengthSquared should work as expected and return exact result.")
        assertApproxEqual (vector2.length (), 53.8516480713450403125, "Vector3 length should work as expected.")
        assertEqual (vector1.distanceSquaredTo (vector2), 1400, "Vector3 distanceSquaredTo should work as expected and return exact result.")
    ...
    public func testLimitingMethods () {
        ...
        assertEqual (Vector3 (x: -5, y: 5, z: 15).clamp (min: Vector3 (), max: vector), Vector3 (x: 0, y: 5, z: 10), "Vector3 clamp should work as expected.")
        assertEqual (vector.clamp (min: Vector3 (x: 0, y: 10, z: 15), max: Vector3 (x: 5, y: 10, z: 20)), Vector3 (x: 5, y: 10, z: 15), "Vector3 clamp should work as expected.")
    }

    public func testNormalizationMethods () {
        assertEqual (Vector3 (x: 1, y: 0, z: 0).isNormalized (), true, "Vector3 isNormalized should return true for a normalized vector.")
        assertEqual (Vector3 (x: 1, y: 1, z: 1).isNormalized (), false, "Vector3 isNormalized should return false for a non-normalized vector.")
        assertEqual (Vector3 (x: 1, y: 0, z: 0).normalized (), Vector3 (x: 1, y: 0, z: 0), "Vector3 normalized should return the same vector for a normalized vector.")
        ...
        //assertEqual (vector, Vector3 (x: 3.2, y: -5.4, z: 6).normalized (), "Vector3 normalize should convert same way as Vector3 normalized.")
    ...
    public func testOperators () {
        ...
        assertEqual ((power1 + power2), Vector3 (x: 1.25, y: 1.625, z: 0.875), "Vector3 addition with powers of two should give exact results.")
        assertEqual ((int1 + int2), Vector3 (x: 5, y: 7, z: 12), "Vector3 addition with integers should give exact results.")
        
        assertApproxEqual ((decimal1 - decimal2), Vector3 (x: 1.1, y: 1.5, z: 2.2), "Vector3 subtraction should behave as expected.")
        assertEqual ((power1 - power2), Vector3 (x: 0.25, y: 1.375, z: 0.375), "Vector3 subtraction with powers of two should give exact results.")
        assertEqual ((int1 - int2), Vector3 (x: 3, y: 3, z: 6), "Vector3 subtraction with integers should give exact results.")
        
        assertApproxEqual ((decimal1 * decimal2), Vector3 (x: 2.76, y: 16.66, z: 43.68), "Vector3 multiplication should behave as expected.")
        assertEqual ((power1 * power2), Vector3 (x: 0.375, y: 0.1875, z: 0.15625), "Vector3 multiplication with powers of two should give exact results.")
        assertEqual ((int1 * int2), Vector3 (x: 4, y: 10, z: 27), "Vector3 multiplication with integers should give exact results.")
        ...
        assertEqual ((power1 / power2), Vector3 (x: 1.5, y: 12.0, z: 2.5), "Vector3 division with powers of two should give exact results.")
        assertEqual ((int1 / int2), Vector3 (x: 4, y: 2.5, z: 3), "Vector3 division with integers should give exact results.")
        
        assertApproxEqual ((decimal1 * 2.0), Vector3 (x: 4.6, y: 9.8, z: 15.6), "Vector3 multiplication should behave as expected.")
        assertEqual ((power1 * 2.0), Vector3 (x: 1.5, y: 3, z: 1.25), "Vector3 multiplication with powers of two should give exact results.")
        assertEqual ((int1 * 2.0), Vector3 (x: 8, y: 10, z: 18), "Vector3 multiplication with integers should give exact results.")
        
        assertApproxEqual ((decimal1 / 2.0), Vector3 (x: 1.15, y: 2.45, z: 3.9), "Vector3 division should behave as expected.")
        assertEqual ((power1 / 2.0), Vector3 (x: 0.375, y: 0.75, z: 0.3125), "Vector3 division with powers of two should give exact results.")
        assertEqual ((int1 / 2.0), Vector3 (x: 2, y: 2.5, z: 4.5), "Vector3 division with integers should give exact results.")
        
        assertEqual (Vector3i (from: decimal1), Vector3i (x: 2, y: 4, z: 7), "Vector3 cast to Vector3i should work as expected.")
        assertEqual (Vector3i (from: decimal2), Vector3i (x: 1, y: 3, z: 5), "Vector3 cast to Vector3i should work as expected.")
        assertEqual (Vector3 (from: Vector3i (x: 1, y: 2, z: 3)), Vector3 (x: 1, y: 2, z: 3), "Vector3 constructed from Vector3i should work as expected.")
        
        assertEqual (Variant (decimal1).description, "(2.3, 4.9, 7.8)", "Vector3 cast to String should work as expected.")
        assertEqual (Variant (decimal2).description, "(1.2, 3.4, 5.6)", "Vector3 cast to String should work as expected.")
        assertEqual (Variant (Vector3 (x: 9.7, y: 9.8, z: 9.9)).description, "(9.7, 9.8, 9.9)", "Vector3 cast to String should work as expected.")
        assertEqual (Variant (Vector3 (x: Float.e, y: Float.sqrt2, z: Float.sqrt3)).description, "(2.718282, 1.414214, 1.732051)", "Vector3 cast to String should print the correct amount of digits for realT = float.")
    }
    ...
    public func testOtherMethods () {
        ...
        assertEqual (vector.snapped (step: Vector3 (x: 1, y: 1, z: 1)), Vector3 (x: 1, y: 3, z: 6), "Vector3 snapped to integers should be the same as rounding.")
        assertEqual (vector.snapped (step: Vector3 (x: 0.25, y: 0.25, z: 0.25)), Vector3 (x: 1.25, y: 3.5, z: 5.5), "Vector3 snapped to 0.25 should give exact results.")
    ...
    public func testPlaneMethods () {
        ...
        assertEqual (vector.bounce (n: vectorY), Vector3 (x: 1.2, y: -3.4, z: 5.6), "Vector3 bounce on a plane with normal of the Y axis should.")
        ...
        assertEqual (vector.reflect (n: vectorY), Vector3 (x: -1.2, y: 3.4, z: -5.6), "Vector3 reflect on a plane with normal of the Y axis should.")
        ...
        assertEqual (vector.project (b: vectorY), Vector3 (x: 0, y: 3.4, z: 0), "Vector3 projected on the Y axis should only give the Y component.")
        ...
        assertEqual (vector.slide (n: vectorY), Vector3 (x: 1.2, y: 0, z: 5.6), "Vector3 slide on a plane with normal of the Y axis should set the Y to zero.")
    ...
    public func testRoundingMethods () {
        ...
        assertEqual (vector1.abs (), vector1, "Vector3 abs should work as expected.")
        assertEqual (vector2.abs (), vector1, "Vector3 abs should work as expected.")
        
        assertEqual (vector1.ceil (), Vector3 (x: 2, y: 4, z: 6), "Vector3 ceil should work as expected.")
        assertEqual (vector2.ceil (), Vector3 (x: 2, y: -3, z: -5), "Vector3 ceil should work as expected.")
        
        assertEqual (vector1.floor (), Vector3 (x: 1, y: 3, z: 5), "Vector3 floor should work as expected.")
        assertEqual (vector2.floor (), Vector3 (x: 1, y: -4, z: -6), "Vector3 floor should work as expected.")
        
        assertEqual (vector1.round (), Vector3 (x: 1, y: 3, z: 6), "Vector3 round should work as expected.")
        assertEqual (vector2.round (), Vector3 (x: 1, y: -3, z: -6), "Vector3 round should work as expected.")
        
        assertEqual (vector1.sign (), Vector3 (x: 1, y: 1, z: 1), "Vector3 sign should work as expected.")
        assertEqual (vector2.sign (), Vector3 (x: 1, y: -1, z: -1), "Vector3 sign should work as expected.")
    }
    ...
    public func testLinearAlgebraMethods () {
        ...
        assertEqual (vectorX.cross (with: vectorY), vectorZ, "Vector3 cross product of X and Y should give Z.")
        assertEqual (vectorY.cross (with: vectorX), -vectorZ, "Vector3 cross product of Y and X should give negative Z.")
        assertEqual (vectorY.cross (with: vectorZ), vectorX, "Vector3 cross product of Y and Z should give X.")
        assertEqual (vectorZ.cross (with: vectorX), vectorY, "Vector3 cross product of Z and X should give Y.")
        ...
        assertEqual (vectorX.dot (with: vectorY), 0.0, "Vector3 dot product of perpendicular vectors should be zero.")
        assertEqual (vectorX.dot (with: vectorX), 1.0, "Vector3 dot product of identical unit vectors should be one.")
        assertEqual ((vectorX * 10.0).dot (with: vectorX * 10.0), 100.0, "Vector3 dot product of same direction vectors should behave as expected.")
    ...
    public func testFiniteNumberChecks () {
        ...
        assertTrue (Vector3 (x: 0, y: 1, z: 2).isFinite (), "Vector3(x: 0, y: 1, z: 2) should be finite")
        
        for x in infinite {
            assertFalse (Vector3 (x: x, y: 1, z: 2).isFinite (), "Vector3 with one component infinite should not be finite.")
            assertFalse (Vector3 (x: 0, y: x, z: 2).isFinite (), "Vector3 with one component infinite should not be finite.")
            assertFalse (Vector3 (x: 0, y: 1, z: x).isFinite (), "Vector3 with one component infinite should not be finite.")
        }
        
        for x in infinite {
            for y in infinite {
                assertFalse (Vector3 (x: x, y: y, z: 2).isFinite (), "Vector3 with two components infinite should not be finite.")
                assertFalse (Vector3 (x: x, y: 1, z: y).isFinite (), "Vector3 with two components infinite should not be finite.")
                assertFalse (Vector3 (x: 0, y: x, z: y).isFinite (), "Vector3 with two components infinite should not be finite.")
            }
        }
        ...
        for x in infinite {
            for y in infinite {
                for z in infinite {
                    assertFalse (Vector3 (x: x, y: y, z: z).isFinite (), "Vector3 with three components infinite should not be finite.")
                }
            }
        }
    }
