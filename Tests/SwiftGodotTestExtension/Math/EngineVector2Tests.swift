...
@SwiftGodotTestSuite
final class EngineVector2Tests {
    public func testConstructorMethods () {
        let vectorEmpty: Vector2 = Vector2 ()
        let vectorZero: Vector2 = Vector2 (x: 0.0, y: 0.0)
        assertEqual (vectorEmpty, vectorZero, "Vector2 Constructor with no inputs should return a zero Vector2.")
    }
    ...
    public func testAxisMethods () {
        ...
        assertEqual (vector.maxAxisIndex (), Vector2.Axis.y.rawValue, "Vector2 maxAxisIndex should work as expected.")
        assertEqual (vector.minAxisIndex (), Vector2.Axis.x.rawValue, "Vector2 minAxisIndex should work as expected.")
    ...
    public func testInterpolationMethods () {
        ...
        assertEqual (vector1.lerp (to: vector2, weight: 0.5), Vector2 (x: 2.5, y: 3.5), "Vector2 lerp should work as expected.")
        ...
        assertEqual (Vector2 ().slerp (to: Vector2 (), weight: 0.5), Vector2 (), "Vector2 slerp with both inputs as zero vectors should return a zero vector.")
        assertEqual (Vector2 ().slerp (to: Vector2 (x: 1, y: 1), weight: 0.5), Vector2 (x: 0.5, y: 0.5), "Vector2 slerp with one input as zero should behave like a regular lerp.")
        assertEqual (Vector2 (x: 1, y: 1).slerp (to: Vector2 (), weight: 0.5), Vector2 (x: 0.5, y: 0.5), "Vector2 slerp with one input as zero should behave like a regular lerp.")
        ...
        assertEqual (vector1.cubicInterpolate (b: vector2, preA: Vector2 (), postB: Vector2 (x: 7, y: 7), weight: 0.5), Vector2 (x: 2.375, y: 3.5), "Vector2 cubicInterpolate should work as expected.")
        ...
        assertEqual (Vector2 (x: 1, y: 0).moveToward (to: Vector2 (x: 10, y: 0), delta: 3), Vector2 (x: 4, y: 0), "Vector2 moveToward should work as expected.")
    }

    public func testLengthMethods () {
        let vector1: Vector2 = Vector2 (x: 10, y: 10)
        let vector2: Vector2 = Vector2 (x: 20, y: 30)
        assertEqual (vector1.lengthSquared (), 200, "Vector2 lengthSquared should work as expected and return exact result.")
        assertApproxEqual (vector1.length (), 10 * Double.sqrt2, "Vector2 length should work as expected.")
        assertEqual (vector2.lengthSquared (), 1300, "Vector2 lengthSquared should work as expected and return exact result.")
        assertApproxEqual (vector2.length (), 36.05551275463989293119, "Vector2 length should work as expected.")
        assertEqual (vector1.distanceSquaredTo (vector2), 500, "Vector2 distanceSquaredTo should work as expected and return exact result.")
    ...
    public func testLimitingMethods () {
        ...
        assertEqual (Vector2 (x: -5, y: 15).clamp (min: Vector2 (), max: vector), Vector2 (x: 0, y: 10), "Vector2 clamp should work as expected.")
        assertEqual (vector.clamp (min: Vector2 (x: 0, y: 15), max: Vector2 (x: 5, y: 20)), Vector2 (x: 5, y: 15), "Vector2 clamp should work as expected.")
    }
    
    public func testNormalizationMethods () {
        assertEqual (Vector2 (x: 1, y: 0).isNormalized (), true, "Vector2 isNormalized should return true for a normalized vector.")
        assertEqual (Vector2 (x: 1, y: 1).isNormalized (), false, "Vector2 isNormalized should return false for a non-normalized vector.")
        assertEqual (Vector2 (x: 1, y: 0).normalized (), Vector2 (x: 1, y: 0), "Vector2 normalized should return the same vector for a normalized vector.")
        ...
        //vector.normalize ()
        //assertEqual (vector, Vector2 (x: 3.2, y: -5.4).normalized (), "Vector2 normalize should convert same way as Vector2 normalized.")
    ...
    public func testOperators () {
        ...
        assertEqual ((power1 + power2), Vector2 (x: 1.25, y: 1.625), "Vector2 addition with powers of two should give exact results.")
        assertEqual ((int1 + int2), Vector2 (x: 5, y: 7), "Vector2 addition with integers should give exact results.")
        
        assertApproxEqual ((decimal1 - decimal2), Vector2 (x: 1.1, y: 1.5), "Vector2 subtraction should behave as expected.")
        assertEqual ((power1 - power2), Vector2 (x: 0.25, y: 1.375), "Vector2 subtraction with powers of two should give exact results.")
        assertEqual ((int1 - int2), Vector2 (x: 3, y: 3), "Vector2 subtraction with integers should give exact results.")
        
        assertApproxEqual ((decimal1 * decimal2), Vector2 (x: 2.76, y: 16.66), "Vector2 multiplication should behave as expected.")
        assertEqual ((power1 * power2), Vector2 (x: 0.375, y: 0.1875), "Vector2 multiplication with powers of two should give exact results.")
        assertEqual ((int1 * int2), Vector2 (x: 4, y: 10), "Vector2 multiplication with integers should give exact results.")
        ...
        assertEqual ((power1 / power2), Vector2 (x: 1.5, y: 12.0), "Vector2 division with powers of two should give exact results.")
        assertEqual ((int1 / int2), Vector2 (x: 4, y: 2.5), "Vector2 division with integers should give exact results.")
        
        assertApproxEqual ((decimal1 * 2.0), Vector2 (x: 4.6, y: 9.8), "Vector2 multiplication should behave as expected.")
        assertEqual ((power1 * 2.0), Vector2 (x: 1.5, y: 3), "Vector2 multiplication with powers of two should give exact results.")
        assertEqual ((int1 * 2.0), Vector2 (x: 8, y: 10), "Vector2 multiplication with integers should give exact results.")
        
        assertApproxEqual ((decimal1 / 2.0), Vector2 (x: 1.15, y: 2.45), "Vector2 division should behave as expected.")
        assertEqual ((power1 / 2.0), Vector2 (x: 0.375, y: 0.75), "Vector2 division with powers of two should give exact results.")
        assertEqual ((int1 / 2.0), Vector2 (x: 2, y: 2.5), "Vector2 division with integers should give exact results.")
        
        assertEqual (Vector2i (from: decimal1), Vector2i (x: 2, y: 4), "Vector2 cast to Vector2i should work as expected.")
        assertEqual (Vector2i (from: decimal2), Vector2i (x: 1, y: 3), "Vector2 cast to Vector2i should work as expected.")
        assertEqual (Vector2 (from: Vector2i (x: 1, y: 2)), Vector2 (x: 1, y: 2), "Vector2 constructed from Vector2i should work as expected.")
        
        assertEqual (Variant (decimal1).description, "(2.3, 4.9)", "Vector2 cast to String should work as expected.")
        assertEqual (Variant (decimal2).description, "(1.2, 3.4)", "Vector2 cast to String should work as expected.")
        assertEqual (Variant (Vector2 (x: 9.8, y: 9.9)).description, "(9.8, 9.9)", "Vector2 cast to String should work as expected.")
        assertEqual (Variant (Vector2 (x: Float.pi, y: Float.tau)).description, "(3.141593, 6.283185)", "Vector2 cast to String should print the correct amount of digits for realT = float.")
    }
    ...
    public func testOtherMethods () {
        ...
        assertEqual (vector.snapped (step: Vector2 (x: 1, y: 1)), Vector2 (x: 1, y: 3), "Vector2 snapped to integers should be the same as rounding.")
    ...
    public func testPlaneMethods () {
        ...
        assertEqual (vector.bounce (n: vectorY), Vector2 (x: 1.2, y: -3.4), "Vector2 bounce on a plane with normal of the Y axis should.")
        ...
        assertEqual (vector.reflect (line: vectorY), Vector2 (x: -1.2, y: 3.4), "Vector2 reflect on a plane with normal of the Y axis should.")
        ...
        assertEqual (vector.project (b: vectorY), Vector2 (x: 0, y: 3.4), "Vector2 projected on the Y axis should only give the Y component.")
        ...
        assertEqual (vector.slide (n: vectorY), Vector2 (x: 1.2, y: 0), "Vector2 slide on a plane with normal of the Y axis should set the Y to zero.")
    ...
    public func testRoundingMethods () {
        ...
        assertEqual (vector1.abs (), vector1, "Vector2 abs should work as expected.")
        assertEqual (vector2.abs (), vector1, "Vector2 abs should work as expected.")
        
        assertEqual (vector1.ceil (), Vector2 (x: 2, y: 6), "Vector2 ceil should work as expected.")
        assertEqual (vector2.ceil (), Vector2 (x: 2, y: -5), "Vector2 ceil should work as expected.")
        
        assertEqual (vector1.floor (), Vector2 (x: 1, y: 5), "Vector2 floor should work as expected.")
        assertEqual (vector2.floor (), Vector2 (x: 1, y: -6), "Vector2 floor should work as expected.")
        
        assertEqual (vector1.round (), Vector2 (x: 1, y: 6), "Vector2 round should work as expected.")
        assertEqual (vector2.round (), Vector2 (x: 1, y: -6), "Vector2 round should work as expected.")
        
        assertEqual (vector1.sign (), Vector2 (x: 1, y: 1), "Vector2 sign should work as expected.")
        assertEqual (vector2.sign (), Vector2 (x: 1, y: -1), "Vector2 sign should work as expected.")
    }
    ...
    public func testLinearAlgebraMethods () {
        ...
        assertEqual (vectorX.cross (with: vectorY), 1, "Vector2 cross product of X and Y should give 1.")
        assertEqual (vectorY.cross (with: vectorX), -1, "Vector2 cross product of Y and X should give negative 1.")
        ...
        assertEqual (vectorX.dot (with: vectorY), 0.0, "Vector2 dot product of perpendicular vectors should be zero.")
        assertEqual (vectorX.dot (with: vectorX), 1.0, "Vector2 dot product of identical unit vectors should be one.")
        assertEqual ((vectorX * 10.0).dot (with: vectorX * 10.0), 100.0, "Vector2 dot product of same direction vectors should behave as expected.")
    ...
    public func testFiniteNumberChecks () {
        ...
        assertTrue (Vector2 (x: 0, y: 1).isFinite (), "Vector2(x: 0, y: 1) should be finite")
        
        for x in infinite {
            assertFalse (Vector2 (x: x, y: 1).isFinite (), "Vector2 with one component infinite should not be finite.")
            assertFalse (Vector2 (x: 0, y: x).isFinite (), "Vector2 with one component infinite should not be finite.")
        }
        
        for x in infinite {
            for y in infinite {
                assertFalse (Vector2 (x: x, y: y).isFinite (), "Vector2 with two components infinite should not be finite.")
            }
        }
    }
