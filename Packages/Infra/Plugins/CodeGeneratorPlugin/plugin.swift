...
@main struct SwiftCodeGeneratorPlugin: BuildToolPlugin {
    func createBuildCommands(context: PluginContext, target: Target) throws -> [Command] {
        ...
        // Locate extension_api.json. In the monolithic layout it lives in this
        // package; in the split (multi-package) layout it lives in the shared
        // Infra package, so we also search our dependency packages for it.
        let api = Self.locateExtensionApi(context: context)
        ...
        if useCombinedOutput {
            ...
            // Always produced by the generator (possibly empty): members of
            // lower-module classes re-surfaced as extensions in this module.
            outputFiles.append(generatedSourcesDir.appending(["generated", "_DeferredExtensions.swift"]))
        }
        ...
        // Provide the module graph so the generator can detect cross-sibling
        // members that no module can host (orphans) and fail loudly.
        let moduleGraphFile = configurationDir.appending(path: "module-graph.json")
        try writeIfChanged(moduleGraphJSON(), to: moduleGraphFile)
        arguments.append(contentsOf: ["--module-graph", moduleGraphFile.path])

        var inputFiles: [URL] = [api, classFilterFile, availableClassFilterFile, builtinFilterFile, moduleGraphFile]
    ...
    /// Finds `Sources/ExtensionApi/extension_api.json`, first in the package that
    /// owns the target being built, then in any (transitive) dependency package.
    /// This lets the split module packages (which don't carry the json) reuse the
    /// copy vended by the shared Infra package.
    static func locateExtensionApi(context: PluginContext) -> URL {
        let relative = ["Sources", "ExtensionApi", "extension_api.json"]
        let fm = FileManager.default

        let local = context.package.directoryURL.appending(relative)
        if fm.fileExists(atPath: local.path) {
            return local
        }

        var seen = Set<Package.ID>()
        var queue = context.package.dependencies.map { $0.package }
        while !queue.isEmpty {
            let pkg = queue.removeFirst()
            guard seen.insert(pkg.id).inserted else { continue }
            let candidate = pkg.directoryURL.appending(relative)
            if fm.fileExists(atPath: candidate.path) {
                return candidate
            }
            queue.append(contentsOf: pkg.dependencies.map { $0.package })
        }

        // Fall back to the local path; the build command will surface a clear
        // missing-input error if it truly does not exist.
        return local
    }
...
""",
                dependencyClassFiles: runtime
                // Core sits below the leaf modules (Controls/2D/3D/...). A handful
                // of core classes reference leaf types in their API surface; those
                // members are omitted from core and re-emitted as extensions in the
...
// Leaf types referenced by core-classified classes, degraded to a core/runtime
// ancestor so they remain owned by their own module's dylib. Seeded by the
// generator's strict reference check; extend as it reports more.
let runtime: [String] = [
    "ClassDB.swift",
    "Object.swift",
    "Engine.swift",
...
let editor: [String] = [
    ...
    // FileSystemDock inherits EditorDock, so it must live in the editor module.
...
// The split module partition + dependency graph, used by the generator to detect
// "cross-sibling" members (whose signature spans two incomparable modules) that no
// module can host — so they fail the build loudly instead of being dropped.
let moduleClassLists: [(String, [String])] = [
    ...
    ("threeD", threeD),
    ("gltf", gltf),
    ("visualShaderNodes", visualShaderNodes),
    ("xr", xr),
    ("editor", editor),
]
...
func moduleGraphJSON() -> String {
    var home: [String: String] = [:]
    for (mod, files) in moduleClassLists {
        for f in files {
            let name = f.hasSuffix(".swift") ? String(f.dropLast(6)) : f
            home[name] = mod
        }
    }
    let graph: [String: Any] = ["deps": moduleDirectDeps, "home": home]
    let data = try! JSONSerialization.data(withJSONObject: graph, options: [.sortedKeys])
