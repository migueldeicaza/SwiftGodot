...
func generateMethods (_ p: Printer,
    ...
    for method in methods {
        // In a split target, omit any member that references a type which isn't
        // available in this module. It is re-emitted as an extension in the
        // higher module that introduces the referenced type (see DeferredSplit).
        guard methodIsEmittableHere(method) else {
            ensureHostableSomewhere(className: cdef.name, memberName: method.name, refs: referencedClassNames(of: method))
            continue
        }
...
/// Resolves the getter/setter method backing a property, walking up the
/// inheritance chain if needed. `methods` is the starting class's own methods.
func findPropertyMethod (forProperty: JGodotProperty, startAt: JGodotExtensionAPIClass, name: String, methods: [JGodotClassMethod], resolvedName: inout String, argName: inout String) -> JGodotClassMethod? {
...
/// The Godot class names referenced by a property's backing getter/setter.
func referencedClassNames(ofProperty property: JGodotProperty, cdef: JGodotExtensionAPIClass) -> Set<String>? {
    var getterName = property.getter
    var argName = ""
    guard !property.getter.isEmpty, !property.getter.starts(with: "_"),
          let getter = findPropertyMethod(forProperty: property, startAt: cdef, name: property.getter, methods: cdef.methods ?? [], resolvedName: &getterName, argName: &argName) else {
        return nil
    }
    var refs = referencedClassNames(of: getter)
    if let psetter = property.setter, !psetter.isEmpty {
        var sn = psetter, san = ""
        if let setter = findPropertyMethod(forProperty: property, startAt: cdef, name: psetter, methods: cdef.methods ?? [], resolvedName: &sn, argName: &san) {
            refs.formUnion(referencedClassNames(of: setter))
        }
    }
    return refs
}

/// Emits one property as a computed `var`. When `emitBackingMethods` is true (the
/// deferred-extension case) the backing getter/setter raw methods are emitted
/// alongside it, since they were omitted from the class body in the lower module.
/// Returns true if the property was emitted.
@discardableResult
func generateProperty (_ p: Printer,
                       cdef: JGodotExtensionAPIClass,
                       property: JGodotProperty,
                       methods: [JGodotClassMethod],
                       referencedMethods: inout Set<String>,
                       asSingleton: Bool,
                       emitBackingMethods: Bool = false) -> Bool
{
    var type: String?
    ...
    var getterName = property.getter
    var gettterArgName = ""
    guard let method = findPropertyMethod (forProperty: property, startAt: cdef, name: property.getter, methods: methods, resolvedName: &getterName, argName: &gettterArgName) else {
    ...
    if let psetter = property.setter {
        setterMethod = findPropertyMethod(forProperty: property, startAt: cdef, name: psetter, methods: methods, resolvedName: &setterName, argName: &setterArgName)
        if setterMethod == nil {
            // Not a bug, but needs to be handled: https://github.com/migueldeicaza/SwiftGodot/issues/67
            return false
        }
    ...
    // In a split target, omit a property whose backing getter/setter references a
    // type not available in this module. It is re-surfaced as an extension
    // computed property in the higher module that introduces the type.
    if !methodIsEmittableHere(method) || (setterMethod.map { !methodIsEmittableHere($0) } ?? false) {
        if let refs = referencedClassNames(ofProperty: property, cdef: cdef) {
            ensureHostableSomewhere(className: cdef.name, memberName: property.name, refs: refs)
        }
        return false
    }

    // In the deferred case the backing raw methods weren't emitted in the lower
    // module's class body, so emit them here. Pass their names in usedMethods so
    // they get the raw, fileprivate form the property accessors call.
    if emitBackingMethods {
        var backingNames: Set<String> = [method.name]
        if let setterMethod { backingNames.insert(setterMethod.name) }
        performExplaniningNonCriticalErrors {
            _ = try generateMethod (p, method: method, className: cdef.name, cdef: cdef, usedMethods: backingNames, generatedMethodKind: .classMethod, asSingleton: asSingleton)
        }
        if let setterMethod {
            performExplaniningNonCriticalErrors {
                _ = try generateMethod (p, method: setterMethod, className: cdef.name, cdef: cdef, usedMethods: backingNames, generatedMethodKind: .classMethod, asSingleton: asSingleton)
            }
        }
    }
                         ...
                         asSingleton: Bool)
{
    ...
    for property in properties {
        generateProperty (p, cdef: cdef, property: property, methods: methods, referencedMethods: &referencedMethods, asSingleton: asSingleton)
    }
}
...
/// Emits a single signal as a computed property. Works both in a class body and
/// in an `extension` (it only uses `self` + the signal name).
func emitSignal (_ p: Printer, cdef: JGodotExtensionAPIClass, signal: JGodotSignal) {
...
func generateSignals (_ p: Printer,
    ...
    for signal in signals {
        // In a split target, omit a signal whose argument types aren't available
        // in this module; it is re-emitted as an extension in the higher module.
        guard signalIsEmittableHere(signal) else {
            ensureHostableSomewhere(className: cdef.name, memberName: "signal " + signal.name, refs: referencedClassNames(of: signal))
            continue
        }
