...
/// The Swift modules that generated Godot binding classes can live in. A class
/// is mangled under the module that defines it, so runtime type lookup must try
/// each. "SwiftGodot" covers the legacy monolith / umbrella; the rest are the
/// split modules.
let swiftGodotBindingModules = [
    "SwiftGodot",
    "SwiftGodotRuntime",
    "SwiftGodotCore",
    "SwiftGodotServers",
...
fileprivate func lookupGodotType(named className: String) -> AnyClass? {
    // A Swift class mangles as <moduleLen><module><classLen><class>C, e.g.
    // "SwiftGodotCore.Node" becomes "14SwiftGodotCore4NodeC". The defining
    // module varies across the split, so try each binding module.
    let candidates: [String] = swiftGodotBindingModules.map {
        "\($0.count)\($0)\(className.count)\(className)C"
    }
