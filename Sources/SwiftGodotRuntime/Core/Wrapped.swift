    ...
    withUnsafePointer(to: &className.content) { classPtr in
        withUnsafePointer(to: &enumName.content) { enumPtr in
            for v in type.allCases {
                // The Godot naming convention is applied (gated by the `automatic_godot_naming_convention` trait).
                let keyString = _convertEnumCaseNameToMatchGodotConvention(String(describing: v))
