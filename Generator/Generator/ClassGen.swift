...
func generateClasses (values: [JGodotExtensionAPIClass], outputDir: String, sharedPrinter: Printer?)  {
    ...
    if sharedPrinter == nil {
        ...
        let _ = Task {
            await withTaskGroup(of: Void.self) { group in
                for cdef in values {
                    group.addTask {
                        processClass (cdef: cdef, outputDir: outputDir, sharedPrinter: sharedPrinter)
                    }
                }
            }
            semaphore.signal()
...
func processClass (cdef: JGodotExtensionAPIClass, outputDir: String, sharedPrinter: Printer?) {
